...
require(['jquery','jqGrid','zDialog','jqtips','jqueryForm','chosen','jqValidate','daterangepicker'],function() {
		...
		$("#addButton").click(function () {
			...
			diag.Drag=true;
			diag.Title ="轮播图新增";
			diag.URL = baseUrl+'/manage/page/banner_add.html';
			diag.Width = 500;
			diag.Height = 300;
		...
		$("#searchButton").click(function () {
			...
			var vstatus= $("#status").val();
			 $(grid_selector).jqGrid("setGridParam", {
			     postData: { name:vsname,typeid:vstype,
		...
		jQuery(grid_selector).jqGrid({
			...
			colNames : ['IDs','轮播图名称', '轮播图','创建时间','操作' ],
			...
			gridComplete: function(){  //添加页面操作按钮
						...
						for(var i=0;i < ids.length;i++){
							...
							var operateString = "<a class='btn btn-success btn-xs' onclick='editRow("+cl+")'> <i class='ace-icon fa ace-icon fa fa-pencil-square-o'></i>修改</a>"; 
								operateString += " <a class='btn btn-warning btn-xs' onclick='deleteBanner("+cl+")'> <i class='ace-icon fa ace-icon fa fa-cog'></i>删除</a>";
